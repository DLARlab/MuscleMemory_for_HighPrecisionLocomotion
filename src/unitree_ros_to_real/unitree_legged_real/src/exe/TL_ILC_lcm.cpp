...
int mainHelper(int argc, char *argv[], TLCM &roslcm)
{
    ...
    int flag_st[4], flag_st_f[4], flag_st_t[4], flag_fly[4], flag_ai, flag_ai0, Force_arrSize, imu_arrSize,torque_arrSize, hd_FR_arrSize, stride_counter, transition_counter;
    int Foot_force[4], Foot_forcex[4], Foot_forcey[4], Foot_forcez[4],joy_butt[4], trigger_R_L[4];
    int trigger_forceF = 0, trigger_forceR = 0;
    float trunk_Vel[3],trunk_Vel_fil[3],trunk_Vel_strongfil[3], trunk_Vel_kf[3],desired_velocity_trunk[3], imu_quaternion[4],angular_Vel[3], imu_acceleration[3],imu_acceleration_fil[3], hd_FR_LA_fil[3];
    float des_trunk_Vel[3];
    float sinr, cosr, sinp, siny, cosy, roll, pitch, yaw;
    float e_mea, e_est, q_var, trunk_YPR_p;
    float param_hip_Kp, param_hip_Kd, param_thigh_Kp, param_thigh_Kd, param_calf_Kp, param_calf_Kd;
    
    float All_joint[12] = {0},twist_trunk[6],All_joint_velfil[12], All_joint_vel[12], trunk_YPR[3], trunk_YPR_kf[3], trunk_YPR_fil[3], imu_angular_vel[3], imu_Gyroscope[3];
...
////////////////parameter setting area///////////////////////////////////////////////////////////////
    ...
    step_time_fly=(float)0.1936;//change automatically
    PD_time = 10000;
    ...
    for (int i_a =0 ; i_a<21; i_a++){
         float a_velocity = (i_a-10.0)/10.0;  
    ...
    for(int i_ini=0; i_ini<4; i_ini++){     
      flag_st[i_ini]=(int)1;
      flag_st_f[i_ini]=(int)0;
      flag_st_t[i_ini]=(int)0;
      flag_fly[i_ini]=(int)0;
      ILC_Switch[i_ini] = (float) 0;
    ...
    for(int i_vel=0; i_vel<3; i_vel++){
        ...
        KalmanFilter kf = KalmanFilter(e_mea, e_est, q_var);
        ...
        if(initiated_flag == true){
         ...
         if( motiontime >= 0){
                ...
                // printf("%f %f \n",  RecvLowROS.motorState[FR_0].mode, RecvLowROS.motorState[FR_1].mode);
                    ...
                    trunk_YPR_p = kf.updateEstimate(trunk_YPR[1]);
                ...
                if( motiontime >= 3000){
                    ...
                    if( motiontime >= 5000){
                        ...
                        if ( flag_st[0] == 1 )
                        {
                           ...
                           // tao_thigh_pitch[0] = Kp_tao_thigh_pitch[0] * (trunk_YPR_fil[1]);
                            ...
                            for ( int i_tor = 0; i_tor < 12; i_tor++ ){
                                if (int_pre_number <=150){        
                                    per_st_Error[i_tor] = Pre_Error_stance[int_pre_number+50][i_tor];
                                    per_st_dError[i_tor] = Pre_dError_stance[int_pre_number+50][i_tor];

                                }
                                else{
                                    per_st_Error[i_tor] = Pre_Error_stance[198][i_tor];
                                    per_st_dError[i_tor] = Pre_dError_stance[198][i_tor];
                                }       
                            }
                            for ( int i_j = 0; i_j < 12; i_j++ )  Feedforward_Error_tor[i_j] = ILC_Switch[0] * (joint_Kp_fd[i_j] * per_st_Error[i_j] + joint_Kv_fd[i_j] * per_st_dError[i_j]) + 0.0 *ILC_Switch[2] * per_st_motor_fil_err_tor[i_j];
                    ...
                    for(int i_leg = 0; i_leg < 2; i_leg++){
                        if (motiontime < PD_time){
                            ...
                            SendLowROS.motorCmd[1 + 3 * i_leg].tau = flag_st[i_leg] * (ILC_Switch[0] * transition_rate * (per_st_motor_total_fil[1 + 3 * i_leg]) + ILC_Switch[0] * transition_rate * Feedforward_Error_sat[1 + 3 * i_leg] + tao_thigh_pitch[i_leg] + ILC_Switch[1] * transition_rate * ILC_torque[1 + 3 * i_leg]);//+ flag_st[i_leg] * transition_rate * Feedforward_Error_sat[1 + 3 * i_leg] //(per_st_motor_total_fil[1 + 3 * 0])//ILC_torque[1 + 3 * i_leg]
                            SendLowROS.motorCmd[2 + 3 * i_leg].tau = flag_st[i_leg] * (ILC_Switch[0] * transition_rate * (per_st_motor_total_fil[2 + 3 * i_leg]) + ILC_Switch[0] * transition_rate * Feedforward_Error_sat[2 + 3 * i_leg] + ILC_Switch[1] * transition_rate * ILC_torque[2 + 3 * i_leg]); //+ flag_st[i_leg] * transition_rate * Feedforward_Error_sat[2 + 3 * i_leg]//(per_st_motor_total_fil[2 + 3 * 0])//ILC_torque[2 + 3 * i_leg]
                        }
                    }
                    for(int i_leg = 2; i_leg < 4; i_leg++){
                        if (motiontime < PD_time){
                            ...
                            SendLowROS.motorCmd[1 + 3 * i_leg].tau = flag_st[i_leg] * (ILC_Switch[0] * transition_rate * (per_st_motor_total_fil[1 + 3 * i_leg]) + ILC_Switch[0] * transition_rate * Feedforward_Error_sat[1 + 3 * i_leg] + tao_thigh_pitch[i_leg] + ILC_Switch[1] * transition_rate * ILC_torque[1 + 3 * i_leg]);// + flag_st[i_leg] * transition_rate * Feedforward_Error_sat[1 + 3 * i_leg]//(per_st_motor_total_fil[1 + 3 * 2]) replaced by ILC_torque[1 + 3 * i_leg] 
                            SendLowROS.motorCmd[2 + 3 * i_leg].tau = flag_st[i_leg] * (ILC_Switch[0] * transition_rate * (per_st_motor_total_fil[2 + 3 * i_leg]) + ILC_Switch[0] * transition_rate * Feedforward_Error_sat[2 + 3 * i_leg] + ILC_Switch[1] * transition_rate * ILC_torque[2 + 3 * i_leg]);//(per_st_motor_total_fil[2 + 3 * 2]);// + flag_st[i_leg] * transition_rate * Feedforward_Error_sat[2 + 3 * i_leg]//(per_st_motor_total_fil[2 + 3 * 2]) /ILC_torque[1 + 3 * i_leg] 
                        }
                    }                    
